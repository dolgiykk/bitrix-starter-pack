...
class module
{
    ...
    {
        return COption::GetOptionString(self::ID, $name, $default);
    }

    public static function setDbOption($name, $value)
    {
        if ($value != COption::GetOptionString(self::ID, $name)) {
            COption::SetOptionString(self::ID, $name, $value);
        }
    }

    public static function removeDbOption($name)
    {
        COption::RemoveOption(self::ID, $name);
    }

    public static function removeDbOptions()
    {
        COption::RemoveOption(self::ID);
    }
    ...
    public static function getModuleDir(): string
    {
        if (is_file(self::getDocRoot() . '/local/modules/' . self::ID . '/include.php')) {
            return self::getDocRoot() . '/local/modules/' . self::ID;
        } else {
            return self::getDocRoot() . '/bitrix/modules/' . self::ID;
        }
    }
    ...
    public static function checkHealth()
    {
        ...
        if (
            is_file($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/' . self::ID . '/include.php')
            && is_file($_SERVER['DOCUMENT_ROOT'] . '/local/modules/' . self::ID . '/include.php')
        ) {
